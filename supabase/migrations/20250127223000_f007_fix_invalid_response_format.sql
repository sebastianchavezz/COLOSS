-- ===========================================================================
-- F007 Hotfix: Fix INVALID response format consistency
-- ===========================================================================
--
-- PROBLEEM: Inconsistente response formats
--
-- HUIDIGE SITUATIE:
-- - VALID scan: {result: 'VALID', ticket: {...}}
-- - INVALID scan: {error: 'INVALID', message: '...'} <- INCONSISTENT
--
-- GEWENSTE SITUATIE:
-- - VALID scan: {result: 'VALID', ticket: {...}}
-- - INVALID scan: {result: 'INVALID', message: '...'} <- CONSISTENT
--
-- RATIONALE:
-- INVALID is geen error, het is een legitieme scan outcome.
-- Echte errors zijn: UNAUTHORIZED, SCANNING_DISABLED, ERROR
-- Scan results zijn: VALID, INVALID, ALREADY_USED, CANCELLED, REFUNDED
--
-- ===========================================================================
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    -- FIX: Return as 'result' not 'error'
  ...
  -- Return result (consistent format for all scan outcomes)
  RETURN jsonb_build_object(
    'result', v_result,
    ...
    'message', CASE
      ...
      WHEN v_result = 'CANCELLED' THEN 'Ticket has been cancelled'
      WHEN v_result = 'REFUNDED' THEN 'Ticket has been refunded'
      ELSE NULL
    END
  );
...
DO $$
BEGIN
  RAISE NOTICE 'F007 Hotfix: scan_ticket response format is now consistent';
  RAISE NOTICE '  - Scan results (VALID, INVALID, ALREADY_USED, etc.) use {result: "..."}';
  RAISE NOTICE '  - Real errors (UNAUTHORIZED, SCANNING_DISABLED) use {error: "..."}';
END $$;
