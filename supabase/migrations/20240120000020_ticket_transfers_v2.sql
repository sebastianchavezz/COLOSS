-- SPRINT 6: TICKET TRANSFERS (V2 - Fixed column references)
-- Migration: 20240120000020_ticket_transfers_v2.sql
-- Replaces: 20240120000020_ticket_transfers.sql

-- ========================================================
-- 0. CLEANUP - Drop old table if it exists with wrong structure
-- ========================================================
...
CREATE TABLE public.ticket_transfers (
    ...
    -- What is being transferred (changed from ticket_instance_id to ticket_id)
    ticket_id uuid NOT NULL REFERENCES public.ticket_instances(id) ON DELETE RESTRICT,
...
CREATE UNIQUE INDEX idx_transfers_unique_pending
    ON public.ticket_transfers(ticket_id)
    WHERE status = 'pending';

CREATE INDEX idx_transfers_ticket ON public.ticket_transfers(ticket_id, status);
CREATE INDEX idx_transfers_from ON public.ticket_transfers(from_participant_id, status);
CREATE INDEX idx_transfers_to_participant ON public.ticket_transfers(to_participant_id, status);
CREATE INDEX idx_transfers_to_email ON public.ticket_transfers(to_email, status);
CREATE INDEX idx_transfers_org_event ON public.ticket_transfers(org_id, event_id, status);
CREATE INDEX idx_transfers_expires ON public.ticket_transfers(expires_at) WHERE status = 'pending';
