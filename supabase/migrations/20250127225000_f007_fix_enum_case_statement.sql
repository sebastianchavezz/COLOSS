-- ===========================================================================
-- F007 Hotfix: Fix enum validation error in CASE statement
-- ===========================================================================
--
-- ROOT CAUSE:
-- CASE statement in scan_ticket gebruikt 'cancelled' en 'refunded' als
-- WHEN values, maar deze bestaan NIET in de ticket_instance_status enum.
--
-- PostgreSQL valideert ALLE enum literals in CASE expressions, zelfs in
-- branches die nooit uitgevoerd worden. Zodra het een ongeldige enum waarde
-- ziet, faalt de hele query tijdens de parse phase.
--
-- ENUM DEFINITIE:
-- ticket_instance_status: 'issued', 'void', 'checked_in'
--
-- FIX:
-- Remove legacy 'cancelled' en 'refunded' WHEN clauses
-- Use only valid enum values: 'issued', 'void', 'checked_in'
--
-- ===========================================================================
...
BEGIN
  ...
  -- ===================================================================
  -- FIX: Map ticket status to scan result
  -- ===================================================================
  -- ONLY use valid ticket_instance_status enum values:
  -- - 'issued'      → VALID (can be scanned)
  -- - 'checked_in'  → ALREADY_USED (already scanned)
  -- - 'void'        → CANCELLED (ticket voided/cancelled)
  --
  -- REMOVED:
  -- - 'cancelled'   → Not in enum (legacy code)
  -- - 'refunded'    → Not in enum (legacy code)
  -- ===================================================================
...
COMMENT ON FUNCTION public.scan_ticket IS
  'F007: Professional ticket scanning - ONLY uses valid ticket_instance_status enum values';
...
DO $$
BEGIN
  RAISE NOTICE '✓ F007: scan_ticket fixed - removed invalid enum values from CASE';
  RAISE NOTICE '  Valid mappings:';
  RAISE NOTICE '    issued      → VALID';
  RAISE NOTICE '    checked_in  → ALREADY_USED';
  RAISE NOTICE '    void        → CANCELLED';
  RAISE NOTICE '  Removed legacy values: cancelled, refunded';
END $$;
