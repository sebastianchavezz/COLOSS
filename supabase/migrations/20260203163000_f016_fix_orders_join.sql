-- ============================================================================
-- F016: Fix orders join - use user_id not participant_id
-- ============================================================================
...
BEGIN
    ...
    -- Get participant's user_id for order lookup
    ...
    SELECT json_build_object(
        ...
        'order', (
            ...
            AND (o.user_id = _user_id OR o.email = p.email)
            AND o.status = 'paid'
        ...
        'history', (
            ...
            FROM audit_log al
            WHERE al.resource_id = p.id::text
            LIMIT 10
        )
    ) INTO _result
    FROM participants p
...
DO $$
BEGIN
    RAISE NOTICE 'F016 fix applied: orders join now uses user_id or email match';
END $$;
