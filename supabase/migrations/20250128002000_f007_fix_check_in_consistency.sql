-- ===========================================================================
-- F007 Hotfix: Fix check_in_consistency constraint violation
-- ===========================================================================
--
-- ERROR: new row for relation "ticket_instances" violates check constraint
--        "ticket_instances_check_in_consistency"
--
-- ROOT CAUSE:
-- De constraint vereist dat checked_in_at en checked_in_by BEIDE gezet zijn
-- of BEIDE NULL zijn. In scan_ticket werden we alleen checked_in_at aan het
-- updaten, maar niet checked_in_by.
--
-- CONSTRAINT:
-- CHECK ((checked_in_at IS NULL AND checked_in_by IS NULL) OR
--        (checked_in_at IS NOT NULL AND checked_in_by IS NOT NULL))
--
-- FIX:
-- Update BEIDE velden in scan_ticket:
-- - checked_in_at = NOW()
...
BEGIN
  ...
  -- Get participant info using correct column names
  ...
  -- ===================================================================
  -- FIX: Atomic update with BOTH checked_in_at AND checked_in_by
  -- ===================================================================
  -- De constraint vereist dat checked_in_at en checked_in_by BEIDE
  -- gezet zijn of BEIDE NULL zijn. We moeten dus beide velden updaten.
  -- ===================================================================
  IF v_result = 'VALID' THEN
    UPDATE ticket_instances
    SET status = 'checked_in',
        checked_in_at = NOW(),
        checked_in_by = auth.uid()  -- FIX: Ook checked_in_by zetten!
...
COMMENT ON FUNCTION public.scan_ticket IS
  'F007: Professional ticket scanning - Fixed check_in_consistency constraint';
...
DO $$
BEGIN
  RAISE NOTICE '✓ F007: scan_ticket fixed - check_in_consistency constraint';
  RAISE NOTICE '  Update sets BOTH:';
  RAISE NOTICE '    - checked_in_at = NOW()';
  RAISE NOTICE '    - checked_in_by = auth.uid()';
  RAISE NOTICE '  This satisfies the constraint:';
  RAISE NOTICE '    CHECK ((checked_in_at IS NULL AND checked_in_by IS NULL) OR';
  RAISE NOTICE '           (checked_in_at IS NOT NULL AND checked_in_by IS NOT NULL))';
END $$;
