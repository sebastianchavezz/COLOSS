...
const VALID_STATUSES = ['open', 'pending', 'closed', 'all']
...
// Thread record as returned from the database
...
// Enriched thread returned to the client
interface ThreadWithParticipant extends ChatThreadRow {
...
serve(async (req: Request) => {
    ...
    try {
        ...
        logger.info('Query params parsed', { eventId, statusFilter, page, pageSize })
        ...
        logger.info('Event resolved', { eventId, orgId, eventName: event.name })

        // =================================================================
        // 4. VERIFY ORGANIZER ROLE (owner/admin/support)
        // =================================================================
        const hasRole = await isOrgMember(supabaseAdmin, orgId, user.id, ['owner', 'admin', 'support'])
        if (!hasRole) {
            logger.warn('User does not have organizer role for this org', { userId: user.id, orgId })
            return errorResponse(
                'Insufficient permissions. Owner, admin, or support role required.',
                'FORBIDDEN',
                403
            )
        }

        logger.info('Organizer role verified', { orgId })
        ...
        threadsQuery = threadsQuery.order('last_message_at', { ascending: false, nullsFirst: false })
        ...
        if (threadsError) {
            logger.error('Failed to query threads', { error: threadsError.message })
            return errorResponse('Failed to fetch threads', 'THREADS_QUERY_ERROR', 500, threadsError.message)
        }

        logger.info('Threads fetched', { count: threads?.length || 0 })
        ...
        if (countError) {
            logger.error('Failed to count threads', { error: countError.message })
        ...
        // =================================================================
        const { data: unreadResult, error: unreadError } = await supabaseAdmin
            .from('chat_threads')
            .select('unread_count_organizer')
            .eq('event_id', eventId)
            .eq('org_id', orgId)
        ...
        if (!unreadError && unreadResult) {
            unreadTotal = unreadResult.reduce(
                (sum: number, row: { unread_count_organizer: number }) => sum + row.unread_count_organizer,
                0
            )
        } else if (unreadError) {
            // Non-fatal: log warning but continue with 0
            logger.warn('Failed to compute unread total (non-fatal)', { error: unreadError.message })
        }

        logger.info('Unread total computed', { unreadTotal })

        // =================================================================
        // 8. ENRICH THREADS WITH PARTICIPANT INFO
        // =================================================================
        const enrichedThreads: ThreadWithParticipant[] = []

        if (threads && threads.length > 0) {
            // Collect unique participant_ids for batch lookup
            const participantIds = [...new Set(
                threads.map((t: ChatThreadRow) => t.participant_id)
            )]
            ...
            if (participantsError) {
                logger.warn('Failed to fetch participant info (using empty placeholders)', { error: participantsError.message })
            }
        ...
        logger.info('Threads enriched with participant info', { enrichedCount: enrichedThreads.length })
        ...
        logger.error('Unexpected error', { message, stack })
